...
const useFirestoreSubscriptions = async ({
  uid,
  subscribeAll,
  unsubscribeAll,
  ...
  const collectionName = getSecretFromCloudFunctions(
    SECRET_KEYS.FEATURE_COLLECTION_NAME
  );
  const documentId = getSecretFromCloudFunctions(SECRET_KEYS.FEATURE_DOCUMENT_ID);
  return Promise.all([collectionName, documentId]).then((data) => {
        ...
        if (!!data) {
          switch (type) {
            case "added":
              ...
              dispatchBadgeCounter({
                type: REDUCER_ACTIONS.BADGE_COUNTER.TAB
                  .SET_BADGE_IN_LOGBOOK_TAB,
                payload: 1,
              });
