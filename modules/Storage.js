...
export const asyncSecureStorage = async ({ action, key, rawValue }) => {
...
export const asyncStorage = async ({ action, key, rawValue }) => {

    // Save Data
    if (action === STORAGE_ACTIONS.SET) {
        try {
            await AsyncStorage.setItem(key, JSON.stringify(rawValue), alert('item saved'));
        } catch (error) {
            alert(error)
        }
    }
